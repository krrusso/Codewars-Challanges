function doneOrNot(board){
	...
	var x = 0;
		for (quadOut=0; quadOut<3; quadOut++){
			for (quadInside=0; quadInside<3; quadInside++){
	...
	for (quadMajor=0; quadMajor<9; quadMajor+=3){
		for (quadRow=0; quadRow<9; quadRow+=3){
			createQuad(quadMajor, quadRow);
		}
	}

	for (boardCol= 0; boardCol<board[0].length; boardCol++){	
		col = board.map(function(value,index) {return value[boardCol]; });
		findDuplicate(col);
	}

	for (boardRow= 0, row =[]; boardRow<board[0].length; boardRow++){
		row[boardRow] = board[boardRow];
		findDuplicate(row[boardRow]); 
	} 
	console.log(match);
	if (board[0].length === 9){return (match === 0 ? 'Finished!' : 'Try again!')}
	//if (board[0].length === )
}
